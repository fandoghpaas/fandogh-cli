...
cluster_url = [key['url'] for key in get_cluster_config().get('clusters') if key['active']][
    0] if get_cluster_config().get('clusters') else None
fandogh_host = cluster_url if cluster_url else os.getenv('FANDOGH_HOST', 'https://api.fandogh.cloud')
